      ...
      <div className="mx-auto flex max-w-6xl flex-col gap-8">
        <header className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
          <div className="float-in">
            <p className="text-xs uppercase tracking-[0.3em] text-slate-500">Aegis</p>
            <h1 className="text-3xl font-semibold text-ink">Sovereign AI Solana Wallet</h1>
            <p className="mt-2 max-w-xl text-sm text-slate-600">
              Every model runs locally. Every decision stays with you. No telemetry, no cloud inference.
            </p>
          </div>
          <div className="float-in-delay flex items-center gap-3">
            <span className="status-pill">
              <span className={`h-2 w-2 rounded-full ${statusTone[qvacStatus]}`}></span>
              {statusCopy[qvacStatus]}
            </span>
            <span className="hidden text-xs text-slate-500 md:inline">Devnet ready</span>
          </div>
        </header>
        ...
        {qvacStatus !== 'ready' && (
          <section className="glass-panel">
            <p className="text-sm font-semibold text-ink">Local models are still warming up.</p>
            <p className="mt-2 text-sm text-slate-600">
              You can explore the UI while QVAC initializes. Once the models are loaded, the AI flows
              activate instantly.
            </p>
            {qvacError && (
